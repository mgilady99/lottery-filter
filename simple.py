...
def apply_consecutive_filter(combo, max_consecutive):
    ...
    for i in range(1, len(sorted_combo)):
        if sorted_combo[i] == sorted_combo[i-1] + 1:
            ...
            current_seq = 1
    return max_seq <= max_consecutive

def apply_even_odd_filter(combo, min_even, max_even):
    evens = sum(1 for x in combo if x % 2 == 0)
    return min_even <= evens <= max_even

def apply_range_filter(combo, low_range, mid_range, high_range):
    # For numbers 1-11, we'll split into just two ranges:
    # Low (1-5) and High (6-11)
    low = sum(1 for x in combo if 1 <= x <= 5)
    high = sum(1 for x in combo if 6 <= x <= 11)
    
    # We'll ignore the mid_range for 1-11 numbers
    # Just check if we have at least 1 number from each range
